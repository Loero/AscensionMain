...
async function addWorkoutEntry() {
  const token = localStorage.getItem("authToken");
  if (!token) {
    alert("Jelentkezz be az edzés követés használatához!");
    return;
  }
  
  const workoutType = document.getElementById("workout-type").value;
  const exerciseName = document.getElementById("exercise-name").value;
  const durationMinutes = parseInt(document.getElementById("workout-duration").value);
  const caloriesBurned = parseInt(document.getElementById("calories-burned").value);
  const sets = document.getElementById("workout-sets").value ? parseInt(document.getElementById("workout-sets").value) : null;
  const reps = document.getElementById("workout-reps").value ? parseInt(document.getElementById("workout-reps").value) : null;
  const weightKg = document.getElementById("workout-weight").value ? parseFloat(document.getElementById("workout-weight").value) : null;
  ...
  if (!workoutType || !exerciseName || !durationMinutes || !caloriesBurned || !date) {
    alert("Töltsd ki a kötelező mezőket (típus, gyakorlat neve, időtartam, kalória, dátum)!");
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      alert("Edzés bejegyzés sikeresen hozzáadva! 🏋️");
      // Mezők törlése
    ...
    console.error("Edzés hozzáadás hiba:", error);
    alert("Hiba történt a mentés során!");
  }
}
...
async function loadWorkoutEntries() {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/workout?startDate=${today}&endDate=${today}`, {
      headers: {
        "Authorization": `Bearer ${token}`
      }
    });
    ...
    if (data.success) {
      renderWorkoutEntries(data.entries);
    }
  } catch (error) {
    console.error("Edzés betöltés hiba:", error);
  }
}
...
function renderWorkoutEntries(entries) {
  ...
  if (!entries || entries.length === 0) {
    list.innerHTML = '<p class="no-data">Még nincs edzés bejegyzés mára.</p>';
    document.getElementById("workout-count").textContent = "(0)";
    document.getElementById("workout-total-duration").textContent = "Időtartam: 0 perc";
    document.getElementById("workout-total-calories").textContent = "Elégetett kalória: 0 kcal";
    document.getElementById("workout-total-count").textContent = "Edzések száma: 0";
    return;
  }
  ...
  const html = entries.map(entry => {
    ...
    if (entry.sets && entry.reps) {
      extraInfo = `${entry.sets}x${entry.reps}`;
    }
    if (entry.weightKg) {
      extraInfo += ` @ ${entry.weightKg}kg`;
    }
    
    return `
      <div class="food-entry">
        <div class="food-entry-title">
          <span class="food-name">${entry.workoutType} - ${entry.exerciseName}</span>
          <span class="food-grams-badge">${entry.durationMinutes} perc</span>
        </div>
        <button class="btn-delete btn-icon" onclick="deleteWorkoutEntry(${entry.id})" aria-label="Tétel törlése">🗑️</button>
        <div class="food-entry-macros">
          <div class="food-chip chip-kcal"><span class="chip-label">Kalória</span> <span class="chip-value">${entry.caloriesBurned} kcal</span></div>
          ${extraInfo ? `<div class="food-chip chip-carbs"><span class="chip-label">Info</span> <span class="chip-value">${extraInfo}</span></div>` : ''}
          ${entry.notes ? `<div class="food-chip chip-fat" style="grid-column: 1 / -1;"><span class="chip-label">Megjegyzés</span> <span class="chip-value">${entry.notes}</span></div>` : ''}
        </div>
      </div>
    `;
  }).join("");
  ...
  // Összesítés
  const totals = entries.reduce((acc, entry) => {
    acc.duration += entry.durationMinutes;
    acc.calories += entry.caloriesBurned;
    acc.count += 1;
    return acc;
  }, { duration: 0, calories: 0, count: 0 });
  
  document.getElementById("workout-count").textContent = `(${totals.count})`;
  document.getElementById("workout-total-duration").textContent = `Időtartam: ${totals.duration} perc`;
  document.getElementById("workout-total-calories").textContent = `Elégetett kalória: ${totals.calories} kcal`;
  document.getElementById("workout-total-count").textContent = `Edzések száma: ${totals.count}`;
}
...
async function deleteWorkoutEntry(id) {
  ...
  if (!confirm("Biztosan törlöd ezt az edzés bejegyzést?")) {
    return;
  }
  ...
  try {
    ...
    console.error("Edzés törlés hiba:", error);
    alert("Hiba történt a törlés során!");
  }
}
