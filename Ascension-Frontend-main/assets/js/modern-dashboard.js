...
function animateProgressBars() {
    ...
    if (dailyProgress) {
        const targetWidth = Math.floor(Math.random() * 40) + 10; // 10-50% progress
    ...
    taskProgressBars.forEach((bar, index) => {
        const targetProgress = 0; // Start from 0
...
function updateProgressLabel(percentage) {
    ...
    if (label) {
        ...
        label.textContent = `Daily Progress – ${completed}/3 completed`;
    }
}

function animateTaskProgressBar(bar, targetProgress, index) {
    const progressText = bar.closest('.task-progress').querySelector('.task-progress-text');
...
function initCrimsonGlow() {
    ...
    const progressBars = document.querySelectorAll('.daily-progress-fill, .task-progress-fill');
